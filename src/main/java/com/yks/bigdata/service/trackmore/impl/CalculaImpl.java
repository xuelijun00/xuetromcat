...
public class CalculaImpl {
    ...
    public ErpOrders processTrackmoreSource(List<JSONObject> jsonObjects, List<LogisticsStatus> statusList, ErpOrders erpOrders, int taskStatus){
        ...
        /*保存各节点时间*/
        List<NodeDto> nodeDtos = erpEOrdersProcessMapper.selectNodes(erpOrders.getErpOrdersId());
        ...
        if(nodeDtos.size() == statusList.size()){
            erpOrders.setTrackStatus(statusList.size());
        }
        return erpOrders;
    }
