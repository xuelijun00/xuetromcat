var channelManagement = {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data) {
                    ...
                    var title=[];
                    var column=[];
                    if($("#form [name='startTime']").val() && $("#form [name='endTime']").val()){
                        title=["平台","国家","渠道","发货日期","付款订单","发货总数","订单执行占比","上网数","上网率(%)","平均上网天数","封发数","封发率(%)","平均封发天数","交航数","交航率(%)","平均交航天数","落地数","落地率(%)","平均落地天数","妥投数","妥投率(%)","平均妥投天数","总重量","平均重量","总运费","平均单价（/g）"];
                        column=["platform","buyerCountry","orderOutTime","channelName","paymentcount","shipcount","orderexecutionPercent","internetcount","internetPercent","internetDate","balecount","balePercent","baleDate","trafficcount","trafficPercent","trafficDate","landingcount","landingPercent","landingDate","deliveredcount","deliveredPercent","deliveredDate","heavi","avgheavi","fee","avgprice"];
                    }else{
                        title=["平台","国家","渠道","付款订单","发货总数","订单执行占比","上网数","上网率(%)","平均上网天数","封发数","封发率(%)","平均封发天数","交航数","交航率(%)","平均交航天数","落地数","落地率(%)","平均落地天数","妥投数","妥投率(%)","平均妥投天数","总重量","平均重量","总运费","平均单价（/g）"];
                        column=["platform","buyerCountry","channelName","paymentcount","shipcount","orderexecutionPercent","internetcount","internetPercent","internetDate","balecount","balePercent","baleDate","trafficcount","trafficPercent","trafficDate","landingcount","landingPercent","landingDate","deliveredcount","deliveredPercent","deliveredDate","heavi","avgheavi","fee","avgprice"];
                    }
    ...
    pageRenderData:function(page,rows){
        if($("#form [name='startTime']").val() && $("#form [name='endTime']").val()){
            $(".appendDateClass").after('<th>发货日期</th>');
        }
        $.getJSON('get', {
            page: page,
            rows: rows,
            ...
            $.each(data.rows,function(index,item){
                ...
                if($("#form [name='startTime']").val() && $("#form [name='endTime']").val()){
                    tr += '<td>'+ item.orderOutTime +'</td>';
                }
                tr += '<td>'+ item.paymentcount +'</td>'+
                    '<td>'+ item.shipcount +'</td>'+
                    '<td>'+ (item.orderexecutionPercent!=''?(item.orderexecutionPercent*100).toFixed(2)+'%':'0%') +'</td>'+
                    '<td>'+ (item.internetcount!=0?item.internetcount.toFixed(2):0) +'</td>'+
                    '<td>'+ (item.internetPercent!=0?(item.internetPercent*100).toFixed(2)+'%':'0%') +'</td>'+
                    '<td>'+ (item.internetDate!=0?item.internetDate.toFixed(1):0) +'</td>'+
                    '<td>'+ (item.balecount!=0?item.balecount:0) +'</td>'+
                    '<td>'+ (item.balePercent!=0?(item.balePercent*100).toFixed(2)+'%':'0%') +'</td>'+
                    '<td>'+ (item.baleDate!=0?item.baleDate.toFixed(1):0) +'</td>'+
                    '<td>'+ (item.trafficcount!=0?item.trafficcount:0) +'</td>'+
                    '<td>'+ (item.trafficPercent!=0?(item.trafficPercent*100).toFixed(2)+'%':'0%') +'</td>'+ //
                    '<td>'+ (item.trafficDate!=0?item.trafficDate.toFixed(1):0) +'</td>'+
                    '<td>'+ (item.landingcount !=0?item.landingcount:0) +'</td>'+
                    '<td>'+ (item.landingPercent!=0?(item.landingPercent*100).toFixed(2)+'%':'0%') +'</td>'+
                    '<td>'+ (item.landingDate!=0?item.landingDate.toFixed(1):0) +'</td>'+
                    '<td>'+ (item.deliveredcount!=0?item.deliveredcount:0) +'</td>'+
                    '<td>'+ (item.deliveredPercent!=0?(item.deliveredPercent*100).toFixed(2)+'%':'0%') +'</td>'+
                    '<td>'+ (item.deliveredDate!=0?item.deliveredDate.toFixed(1):0) +'</td>'+
