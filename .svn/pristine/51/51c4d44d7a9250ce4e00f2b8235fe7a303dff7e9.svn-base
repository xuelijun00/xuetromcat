	...
	public MessageVo upload(MultipartFile file){
    	try{
			...
			for (int i = 1;i<records.size();i++){
                ...
                if (records.get(i).get(7) != null && !records.get(i).get(7).equals("")) {
                    exceptionStatus = Integer.parseInt(records.get(i).get(7));
                }
                if(StringUtils.isNotEmpty(trackCode) && StringUtils.isNotEmpty(remark)){
                    ...
                    } else if (exceptionStatus != 0) {//本来不是异常的标记为异常
                        erpOrdersService.updateExceptionMessage(trackCode, remark, exceptionStatus);
                    } else {
