...
public class LogisticsDetailsController {
    ...
    @RequestMapping(value = "/getOrderToUpdate",method = RequestMethod.GET)
    public String getOrderToUpdate(String ordersMailCode,HttpServletRequest request) {
        ErpOrders orders = processService.selectOrderDetailByOrdersMailCode(ordersMailCode);
        request.setAttribute("orders",orders);
        return "trackmore/order_process_add_or_edit";
    }
    @ResponseBody
    @RequestMapping(value = "/updateOrderTrackTime")
    public MessageVo updateOrderOutTimeAndTrackTime(String erpOrdersId,String ordersMailCode,String platform,String country,String channelName,String ordersOutTime,String internetDate,String baleDate,String trafficDate,String landingDate,String deliveredDate) {
        ErpOrders order = new ErpOrders();
        order.setErpOrdersId(Long.valueOf(erpOrdersId));
        order.setOrdersMailCode(ordersMailCode);
        order.setPlatform(platform);
        order.setCountry(country);
        order.setChannelName(channelName);
        try {
            order.setOrdersOutTime(DateUtils.formmatStr(ordersOutTime));
            order.setInternetDate(DateUtils.formmatStr(internetDate));
            order.setTrafficDate(DateUtils.formmatStr(trafficDate));
            order.setBaleDate(DateUtils.formmatStr(baleDate));
            order.setLandingDate(DateUtils.formmatStr(landingDate));
            order.setDeliveredDate(DateUtils.formmatStr(deliveredDate));
        } catch (Exception e) {
            return new MessageVo(HttpStatus.SC_BAD_REQUEST, "时间格式错误");
        }
        processService.updateOrderTrackStatus(order);
        return new MessageVo(HttpStatus.SC_OK, "修改成功");
    }
}
